<h1 align="left">Hi 👋, I'm Suraj</h1>
<h3 align="center">Pentester & CloudSec</h3>
<img align="right" alt="Coding" width="400" src="https://64.media.tumblr.com/b2db124c402f36f256fdf0f0a21820d4/9183c63c3a6737f8-31/s540x810/e28754840841b423c0c4a91e24f303c5dcc7c0a5.gif">

<p align="left"> <img src="https://komarev.com/ghpvc/?username=0xmrsecurity&label=Profile%20views&color=0e75b6&style=flat" alt="0xmrsecurity" /> </p>

- 🔭 I’m currently working in Pentesting **Web, Active Directory, AWS and AI/LLM**
- 🌱 I’m currently learning **Web and AWS**
- 👯 I’m looking to collaborate on Github **https://github.com/0xmrsecurity**
- 👨‍💻 All of my blog are available at [https://0xmr.qzz.io/](https://0xmr.qzz.io/)
- 💬 Ask me about Pentesting **AD, Web, AWS and AI/LLM**
- 📫 How to reach me **Su8aj@protonmail.com**


<!-- About Me Section with Modern Cards -->
## <img src="https://media.giphy.com/media/iY8CRBdQXODJSCERIr/giphy.gif" width="35"> About Me
 
###  Who Am I?
```bash
- Cybersecurity undergraduate dedicated to building secure systems and identifying vulnerabilities before they become risks for businesses.
- Experienced in web application testing, Active Directory exploitation, AWS cloud security, and emerging AI/LLM security testing.
- Disclosed exposed S3 buckets and identified IDOR vulnerabilities in live production systems.
- Discovered critical AI/LLM vulnerabilities, including command injection in Grok and system prompt extraction in the Chinese AI model Kimi.
- Actively contribute to open-source security tooling on GitHub. Not just studying the field — operating in it.
```


<!-- Featured Project Section -->

## <img src="https://media.giphy.com/media/WFZvB7VIXBgiz3oDXE/giphy.gif" width="35"> Featured Project
> Netspray:-  NetSpray is a wrapper script designed to save time when performing password or hash spraying across multiple protocols. It leverages the power of NetExec to automate the process efficiently.

[live_Project_link](https://github.com/0xmrsecurity/netspray)

```bash
Usage: NetSpray <protocols|all> <targets|subnet> -u <username> [-p <password> | -H <hash>] [OPTIONS]
```
> Scrad:-  This tool finds hidden endpoints across the entire website using js.

[live_Project_link](https://github.com/0xmrsecurity/Scrad)
```bash
Usage Ready to use from brower, just click on scrad bookmark and it will open a new page with in a 2 seconds.
```
> Public POC (Proof of Concepts):- list of public Exploit in python and bash languages.

[live_Project_link](https://github.com/0xmrsecurity/Public_Poc)

> Pentesting Notes Site

[0xmrsecurity.github.io](https://0xmrsecurity.github.io/)

### <img src='https://media1.giphy.com/media/du3J3cXyzhj75IOgvA/giphy.gif?cid=ecf05e47x2g034i9pzwtzzsd3xgg2w9nr94t4tflbbgo3008&rid=giphy.gif' width='25' /> My Github Stats:
![Apoorv's github streak](https://streak-stats.demolab.com?user=apoorvtyagi&theme=dark&mode=weekly)

<!--START_SECTION:waka-->
![Code Time](http://img.shields.io/badge/Code%20Time-280%20hrs%204%20mins-blue?style=flat)

-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
## ✍️ Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

---
[![](https://visitcount.itsvg.in/api?id=5epi0l&icon=1&color=0)](https://visitcount.itsvg.in)

### 🔝
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/tobiasmeyhoefer/tobiasmeyhoefer/output/github-snake-dark.svg" />
  <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/tobiasmeyhoefer/tobiasmeyhoefer/output/github-snake.svg" />
  <img alt="github-snake" src="https://raw.githubusercontent.com/tobiasmeyhoefer/tobiasmeyhoefer/output/github-snake.svg" />
</picture>

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
